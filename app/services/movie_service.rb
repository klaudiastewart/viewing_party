class MovieService
  ...
  def self.list_movies_by_keyword(query)
    page_1= conn.get("/3/search/movie?api_key=#{ENV['MOVIE_TOKEN']}&query=#{query}")
    page_1_json = JSON.parse(page_1.body, symbolize_names: true)
    movie_list = page_1_json[:results]
  end

  def self.return_single_movie(id)
    selected_movie = conn.get("/3/movie/#{id}?api_key=#{ENV['MOVIE_TOKEN']}&language=en-US")
    JSON.parse(selected_movie.body, symbolize_names: true)
  end

  def self.return_single_movie_review(id)
    reviews = conn.get("/3/movie/#{id}/reviews?api_key=#{ENV['MOVIE_TOKEN']}&language=en-US&page=1")
    JSON.parse(reviews.body, symbolize_names: true)
  end
  def self.return_single_movie_cast(id)
    cast = conn.get("/3/movie/#{id}/credits?api_key=#{ENV['MOVIE_TOKEN']}&language=en-US&page=1")
